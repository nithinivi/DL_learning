...
def getSortedFiles(allFiles, upTo=None):
    '''Returns all the notebok files sorted by name.
       ...
       The sorting optioj is important to ensure that
    the notebok are executed in correct order.
    '''
    ret = []
    if allFiles is True:
        ret = glob.glob(
            '*.ipynb')  # Checks both that is bool type and that is True
